...
def test(GAT_model, test_dataloader, data_split='valid', save_pred=False, bestacc=0,files=None,bondfeatures= None):
    ...
    if bestacc < acc and bestacc:
        ...
        torch.save(GAT_model.state_dict(),
                   'checkpoints/{}best_checkpoint.pt'.format(args.exp_name))
...
if __name__ == '__main__':
    ...
    args.exp_name += "_egat_dvml_number_{}".format(numlayer)

    print(args)
    test_id = '{}'.format(args.logdir)
    filename = 'logs/' + test_id + args.exp_name + '.csv'
    sk_filename = 'logs/' + test_id + args.exp_name + '.txt'
    ...
    if args.load:
        ## if you want to load trained pt, add filepath in the following code.
        GAT_model.load_state_dict(
            torch.load('./yourtrianfile.pt',
